...
rule MSA2GFA: # Converts the multi-sequence alignment into a FGA format
    ...
    Genomes/msa_to_gfa/msa_to_gfa/main.py -f {input[0]} -o {output[0]} --log test.log"""
rule cleanGFA:
    input:"MSA2GFA.gfa"
...
rule Delete:
    input:  "%s_Multi_Seq_Align.aln" %end 
    output: "DONE.txt"
    shell: """rm *_parsed_Final.fa && rm *_parsed_final.fa && rm *_results_test_2.txt && touch {output[0]} """