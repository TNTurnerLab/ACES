...
1. <h4> '*_Multi_Seq_Align.fa' </h4> ([Rule muscle](#RM))
    
	1. Generated results from rule ‘muscle’.

2. <h4> '*SPECIESNAME*_blast_results.txt' </h4> ([Rule BLAST](#RB))
    
	2. Should be generated and moved to archived directory

3. <h4> '*_Files_Generated_Report.txt' </h4> ([Rule genratedReport](#RGR))
    
	3. Report of all files created from rule ‘BLAST’ and if their hits were significant or not.

4. <h4> '*_Parsed_Final.fa' </h4> ([Rule qInput](#RQI))
...
5. <h4> '*_NameKey.txt' </h4> ([Rule keyDoc](#RKD))
